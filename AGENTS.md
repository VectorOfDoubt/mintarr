...
Baseline: ADR-001 v1.3
Release: v1.3.2
Source revision: 582b7e4

## Architecture baseline — rules for AI agents

This project follows **ADR-001 (robust, vendor-independent Git architecture)**,
**ADR-002 (agent instruction distribution)**, and where explicitly activated
**ADR-003 (GitHub collaboration profile)**. Agents MUST respect the rules below. Each rule
is tagged with its criticality. These are guidance; the security/data/legal-critical rules
are ALSO enforced technically — never assume a text rule is the enforcement.

1. **[SECURITY] One declared writer-of-record per repository.** `FORGEJO-FIRST` is the
   default: Forgejo is authoritative for Git refs and metadata, and `origin` is Forgejo.
   `GITHUB-COLLAB` is allowed only for an explicitly approved, `PUBLIC-SAFE` repository
   under ADR-003; while active, GitHub is the declared writer for refs and collaboration
   metadata and Forgejo is its read-only recovery replica. Never assume a profile from a
   remote name: check the repository declaration. Never allow both platforms to write the
   same protected ref.
...
2. **[DATA] Mirrors are not backup.** Git mirrors carry commits/branches/tags only — not
   ...
   propose a mirror as a substitute for backup. If an approved external platform is
   authoritative for collaboration metadata, that metadata must be exported versioned,
   integrity-checked, retained, and restore-tested inside the owner's backup domain.

3. **[DATA] Instance-managed mirrors, never local multi-push for redundancy.**
   `FORGEJO-FIRST` fans out through Forgejo server-side push mirrors.
   `GITHUB-COLLAB` uses a Forgejo server-side pull mirror from its declared GitHub writer
   and must not push-mirror back to that writer. Never add multiple client push URLs as
   the redundancy mechanism. Mirror lag and divergence must fail closed and alert.
...
10. **[SECURITY] GitHub is optional at architecture level and gated per repository.**
    `FORGEJO-FIRST` works fully without GitHub. A `GITHUB-COLLAB` repository may depend on
    GitHub for its active collaboration UX only after written approval and account
    resolution; it must retain a tested path to fail over code development to Forgejo.
    Do not activate the profile merely because a GitHub remote exists.
