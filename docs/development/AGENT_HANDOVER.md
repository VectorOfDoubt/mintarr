...
- **Soulseek target-album guard is merged and deployed** (`37243e6`, PR #23). `pipeline.execute_source_grab` now threads optional `target_album_id` into `_trigger_lidarr_import`, and Soulseek imports run a pre-ManualImport guard. Follow-up work after dogfood found Lidarr `history`/queue exposes the targeted `albumId` for Mintarr grabs; Mintarr now uses that hard target id when available, with title compatibility as fallback. If Lidarr resolves a Soulseek candidate to a different album, Mintarr marks the job import-failed and cleans the Lidarr queue before posting `ManualImport`.
...
1. Dogfood the hard `albumId` target guard after deploy, then decide whether Soulseek can move from manual/observed grabs to broader automatic use.
