...
- **F5.4 Library evidence index** — measure existing library files with the same sensor model and compare candidate vs *measured* existing quality, instead of trusting Lidarr's quality label ([F5.4 design](../design/F5.4_LIBRARY_EVIDENCE_INDEX.md)). Read-only; Lidarr stays identity/ownership truth, Mintarr becomes quality truth. On-demand per-album measurement first, background full-library index later; default-safe rollout.
