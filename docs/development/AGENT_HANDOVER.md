...
- **Local Docker runtime cutover is in progress.** As of 2026-05-31 19:42 Europe/Berlin, `mintarr` runs image `mintarr:local` on `127.0.0.1:5025->8000` using the old TidalHires mounts/env. The old container is preserved but stopped as `tidalhires-legacy-20260531-194210`. Backup is under `/home/esj006/backups/mintarr-cutover-20260531-194115`. Do not delete the legacy container, token/config backup, state, or sidecars until the new Mintarr runtime has passed 2-5 dogfood imports.
- **Dogfood status after cutover:** Lidarr `indexer/test` and `downloadclient/test` pass against Mintarr. Two TIDAL dogfood grabs completed safely but were blocked before import by the hard codec gate: `bb865f7d0fb8` (`Aphex Twin - Windowlicker`) and `8f47fad966b1` (`Andrea Bocelli - Season of Champions`). Both received AAC/non-FLAC from TIDAL despite being advertised as FLAC 24bit. This validates the gate, but does not count as a successful import. Investigate TIDAL/Radexito HiRes delivery before deleting `tidalhires-legacy-*` or treating cutover as complete.
