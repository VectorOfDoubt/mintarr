...
| `MINTARR_LIBRARY_SPECTRAL` | Opt-in (F5.4 slice 4a): also run FLAC Detective on existing library files to measure **authenticity** (genuine vs measured-fake), per-trackfile and freshness-cached. Heavier than the ffprobe tier; requires the Detective service to mount the library read-only at the same path Mintarr resolves. Record-only today (surfaced in the drawer, no decision effect yet). Requires `MINTARR_LIBRARY_ROOT`; default-off. | `false` |
