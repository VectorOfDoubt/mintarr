...
| `ALBUM_LEVEL_CANCEL_HOLD.md` | proposed | Codex | P1 | Design for short-lived album-scoped holds after operator cancel/remove, preventing immediate same-album re-grab cascades while preserving per-release blocklisting. |
...
| 0.24 | 2026-06-08 | **F5.4 library evidence index — measured existing quality drives decisions (opt-in).** Mintarr becomes the quality truth over Lidarr's label: design ([F5.4](design/F5.4_LIBRARY_EVIDENCE_INDEX.md)) + ADR-0008 amendment (read-only quality register, no library management) + slices: read-only ffprobe/`flac -t` measurement of existing library files into a `library_evidence` index with a locked path/size/mtime/sensor-version freshness basis, record-only import-time wiring, dashboard visibility (basenames only), the quality-vector comparison (validity → authenticity → lossless tier → completeness), and **opt-in default-off** decision use (`MINTARR_MEASURED_EXISTING`) acting only on freshness-validated evidence (else label fallback). Remaining: spectral tier on existing files, explicit unknown-tier abstain, background full-library index. |
| 0.23 | 2026-06-07 | **F5.3B network CTDB/AccurateRip advisory lane shipped.** Design ([F5.3B](design/F5.3B_CTDB_NETWORK_VERIFIER.md)) plus slices: CD-TOC reconstruction, AccurateRip/CDDB/CTDB disc-id + lookup conventions **validated against the live DBs** (frozen offline fixtures), cached/timeout-bounded AccurateRip + CTDB lookup clients (disc-identity-checked), a default-off `ctdb` verifier connector, and an advisory `ctdb` sensor result emitted when enabled. Lookup-only and decision-neutral; sub-slice B (PCM decode + checksum recompute → opt-in decision) remains. |
