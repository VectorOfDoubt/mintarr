...
- **F5.4 Library evidence index** — measure existing library files with the same sensor model and compare candidate vs *measured* existing quality, instead of trusting Lidarr's quality label ([F5.4 design](../design/F5.4_LIBRARY_EVIDENCE_INDEX.md); [ADR-0008 amendment](../architecture/adr/0008-strategic-positioning.md)). Read-only; Lidarr stays identity/ownership truth, Mintarr becomes quality truth. **Shipped:** on-demand per-album measurement (ffprobe + `flac -t`) into a `library_evidence` index, dashboard visibility, the quality-vector comparison (validity → authenticity → lossless tier → completeness), explicit unknown-tier abstain, FLAC Detective spectral authenticity for existing files, **opt-in default-off** decision use (`MINTARR_MEASURED_EXISTING`, plus `MINTARR_LIBRARY_SPECTRAL` for authenticity) that only acts on freshness-validated evidence and otherwise falls back to the label, an operator-triggered cheap full-library scan, a read-only ranking view, default-off scheduled cheap scans that only queue when imports are idle, and a default-off `spectral_missing` background mode with cheap-fresh prefilter, item-ledger resume/skip, and import-priority pause before each Detective request. **In flight:** a metadata/integrity/spectral scan-tier split ([F5.4 scan tiers](../design/F5.4_SCAN_TIERS.md)) so a fast ffprobe-only metadata pass gives the library overview while heavy `flac -t` integrity and FLAC Detective stay separate, evidence-levelled tiers (unknown integrity/authenticity is never read as OK). **Remaining:** evidence-level ranking + conservative known-ness policy in `compare()`; richer scan UI/ETA; and an **adaptive (auto) scan-concurrency mode** — measure live per-tier throughput/latency and hill-climb the worker count to the optimum for the actual storage (high on fast NVMe/SSD where it is CPU-bound, low on a NAS where it is I/O-bound), bounded by the container's real available CPUs (cgroup/affinity-aware, not host `cpu_count`), with the explicit per-tier defaults as the override/fallback. The current fixed defaults (cheap `min(4, cpu/4)`, metadata 8, integrity 4) are a conservative NAS-derived baseline, not a measured optimum.
